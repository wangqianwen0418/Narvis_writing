...
\textbf{A visual primitive} is one graphic element whose visual channels, such as color, width, height, are mapped to data attributes with certain visual grammar. \siwei{what is graphic element? have you defined before? cite papers here} For instance, a point whose size and color are encoded \siwei{are encoded to what? the sentence is broken.} is a visual primitive. Size is a visual channel, and``size indicates the importance score'' is a visual grammar. \siwei{Please define visual grammar before using an example. Further, please distinguish visual channel and visual grammar here.}

\textbf{A visual unit} is the assembly of visual primitives based on a certain construction rule, as Table~\ref{tab:unit} shows. 
%For the taxonomy of constructing rule, we refer the ``alignment method'' in \cite{kucher2015text} but make it more specific. 
A visual primitive can form different visual units by following different constructing rule \siwei{you use construction rule before. Please be consistent. Further, maybe ``layout'' is a better word?}. For example, the visual primitive, dot, can assemble \siwei{the word assemble used here seems not correct. use ``form'' or sth similar} scatter plot, spiral dot chart, or circle packing chart by following radial, orthogonal, or metric-based construction rule, respectively. \original{We are not pretending that our table includes all existing visual units, since new design is proposed constantly.}\siwei{say what you have done, rather what you do not pretend to do. please say sth like: we selectively list several visual units in Table 1.} A visual unit is the smallest functional unit of a visualization. 
\original{Note that we only consider statical visualization. } \siwei{this is redundant here, mention it in Discussion part}
...
We first describe the relationship between conceptual compositions, then offer suggestion for narrative sequence based on these relationships. \original{Notice that we skip the relationship between viusal primitives since there is only one kind of visual primitive in a visual unit. }\siwei{this is wrong because you mentioned a visual unit can contain two visual primitives. do you mean there is on kind of relationship among visual primitives?}
