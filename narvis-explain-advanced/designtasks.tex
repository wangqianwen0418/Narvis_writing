...
\siwei{this example is problematic. focus+context is used for visualizing data, not for presenting visualization. why users should use this technique in presenting visualization? You should list an example that is a MUST (and time-consuming) in presenting visualization, such as decomposition.} \original{For example, "focus + context" techniques are widely used in data visualization to guide the user’s attention to the region of interest \cite{doleisch2003interactive, kosara2002focus+}. It requires exaggeration or suppression of the visual channels, like hue, luminance , sharpness, or size, of graphical elements, which are hard to perform in these common presentation tool. }
...
People with extensive experience in designing data visualization might have little knowledge about how to give a comprehensive presentation of a visual design \siwei{what is your point? comprehensive or presentation? Options seem have no relations with comprehensive.}. 
...
\original{A comprehensive slideshow should eliminate such omission. }\siwei{This sentence is strange. Rewrite it by emphasizing the importance of a comprehensive slideshow.}

\textbf{DA6. Keep the sense of overview}
Grouping slides into sections, and inserting visual notice \siwei{what is visual notice? a well-defined term?}, like a progress bar, to indicate the overall structure is an effective and widely used presentation skill \siwei{if you say they are widely used, please cite}. However, partly due to the lack of a clear narrative structure, this technique is rarely used in the presentation of a visualization, in spite of the demand from audiences. \siwei{do not say the technique is rarely used in visualization because it is not convincing. say how the technique can help visualization presentation, therefore it is important.}
